...
const fetchAndStoreData = async () => {
    try {
        ...
        if (error.response) {
            console.error(`❌ Erreur lors de la récupération des données : ${error.response.status} ${error.response.statusText}`);
        } else if (error.request) {
            console.error("❌ Erreur lors de la récupération des données : Pas de réponse reçue");
        } else {
